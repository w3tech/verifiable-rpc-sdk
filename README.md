...
3. Reconstructs the 80-byte canonical pre-image (`chain_id (8B LE) || sha256(request_body) (32B) || sha256(response_body) (32B) || timestamp_ms (8B LE)`).
4. Verifies the Ed25519 signature against the pubkey — any failure becomes a typed error.
5. Enforces a client-side replay window on `vRPC-Timestamp` (default 60s).
...
In progress under Jira [SHARK-3283](https://w3tech.atlassian.net/browse/SHARK-3283).

Pairs with sidecar [`v0.1.0`](https://github.com/w3tech/verifiable-rpc-sidecar/releases/tag/v0.1.0).

Note: real TDX hardware deploy is not done yet. The SDK is currently developed against a sidecar exercised by the Phala dstack simulator. Hardware deploy + compose-hash registry are tracked separately.

## Scope

| Area | Deliverable |
|------|-------------|
| Wire format | TypeScript types matching the nested `attestation.quote` wire end-to-end |
| Signature verification | Ed25519 verify over the 80-byte pre-image, per-call |
...
Out of scope for this ticket (separate sibling work):

- TDX quote → compose-hash registry verification
