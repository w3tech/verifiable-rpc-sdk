...
import { DecodeFailedError, UnsupportedEncodingError } from "./errors";
 ...
 * 23.8; older runtimes (e.g. CI Node 22.14) lack it, in which case a zstd
 * upstream response fails closed as UnsupportedEncoding.
 */
...
function resolveCodec(coding: string): AsyncCodec {
  switch (coding) {
    ...
    case "zstd":
      ...
      throw new UnsupportedEncodingError(coding);
    default:
      throw new UnsupportedEncodingError(coding);
  }
}
 ...
 * Unknown/unavailable coding → UnsupportedEncodingError; decode failure
 * (including the output cap) → DecodeFailedError. Both fail closed upstream.
 */
